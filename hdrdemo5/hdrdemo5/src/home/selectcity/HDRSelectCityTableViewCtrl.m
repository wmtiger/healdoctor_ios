...
@interface HDRSelectCityTableViewCtrl ()

@end

@implementation HDRSelectCityTableViewCtrl
...
- (void)viewDidLoad
{
    ...
    _cities = [HDRGlobalData instance].cityDict;
    _keys = [HDRGlobalData instance].cityKeys;
    
//    NSLog(@"citys %@", _cities.allKeys[0]);
//    NSLog(@"keys %d", [_keys count]);
...
{
    NSString *key = [_keys objectAtIndex:indexPath.section];
    NSString *titileString = [[_cities objectForKey:key] objectAtIndex:indexPath.row];
    
    [_selectDelegate selectedCityName:titileString];
    [self closeSelectView];
